...
export class SorobanTypeConverter {
  ...
  static toU64(value: bigint): xdr.ScVal {
    return xdr.ScVal.scvU64(
        new xdr.Uint64(BigInt.asUintN(64, value))
    );
  }
  ...
  static toEnum(key: EnumKey): xdr.ScVal {
    return xdr.ScVal.scvU32(key);
  }

  static toStruct(data: StructData): xdr.ScVal {
    const entry1 = new xdr.ScMapEntry({key: xdr.ScVal.scvSymbol('n'), val: this.toU32(data.n)});
    const entry2 = new xdr.ScMapEntry({key: xdr.ScVal.scvSymbol('s'), val: this.toString(data.s)});
    const entries : xdr.ScMapEntry[] = [entry1, entry2];
  ...
  static stringNumberMapToMap(map: Map<number, string>): xdr.ScVal {
    const entries: xdr.ScMapEntry[] = Array.from(map.entries()).map(
      ([key, value]) => new xdr.ScMapEntry({key: this.toU32(key), val: this.toString(value)})
    );
    return this.toMap(entries);
  }
